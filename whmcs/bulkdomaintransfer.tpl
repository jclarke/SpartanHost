...
{if $invalid}
	<p class="fontsize3 domcheckererror text-center">{$LANG.domaincheckerbulkinvaliddomain}</p>
{/if}
...
{foreach from=$availabilityresults key=num item=result}
        <tr>
            <td class="text-center">{if $result.status eq "unavailable"}<input type="checkbox" name="domains[]" value="{$result.domain}" {if $num eq "0" && $available}checked {/if}/><input type="hidden" name="domainsregperiod[{$result.domain}]" value="{$result.period}" />{else}X{/if}</td>
            <td>{$result.domain}</td>
            <td class="text-center {if $result.status eq "unavailable"}domcheckersuccess{else}domcheckererror{/if}">{if $result.status eq "unavailable"}{$LANG.domaincheckeravailtransfer}{else}{$LANG.domainunavailable}{/if}</td>
            <td class="text-center">{if $result.status eq "unavailable"}<select name="domainsregperiod[{$result.domain}]">{foreach key=period item=regoption from=$result.regoptions}{if $regoption.transfer}<option value="{$period}">{$period} {$LANG.orderyears} @ {$regoption.transfer}</option>{/if}{/foreach}</select>{/if}</td>
        </tr>
{/foreach}
</table>
